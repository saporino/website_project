...
### Unidade 9.1 — Mercado Pago marketplace em ambiente de teste
**STATUS: CONCLUÍDA NA EXECUÇÃO, aguardando aprovação do PM** · Detalhe técnico: RAIO-X §17.1.13

**Decisões materiais (com base na documentação oficial consultada em 14/09/2026):**
- **Split 1:1.** O Mercado Pago disponível no Brasil divide cada pagamento com UM vendedor; o 1:N depende de contrato comercial. Por isso **cada loja é um pagamento separado dentro do mesmo pedido**: um checkout para o comprador, mas uma cobrança por loja (dois Pix quando há duas lojas).
- **Comissão do Coffee LiVRE** vai na própria transação (`application_fee` = comissão + tarifa operacional + frete do CD).
- **Taxa do Mercado Pago:** é descontada do pagamento do vendedor, **inclusive sobre o frete**. A base exata não está confirmada: fica estimada e é corrigida pela reconciliação. **Decisão comercial pendente:** o Coffee LiVRE compensa ou não o vendedor pela taxa sobre o frete (hoje não compensa).
- **Vendedor precisa conectar a própria conta Mercado Pago** (OAuth) e ter KYC nível 6 para receber.
- **Produção: pagamento online DESATIVADO** até a U9.2. Mock só existe em staging.
