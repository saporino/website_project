...
```
- **O hero é uma ABERTURA (gate), não o topo de uma página.** Em `/` renderiza-se **só o hero** — sem header, sem nada abaixo — então o documento termina no fim do hero e **o scroll pára ali**. **Só o clique em "CONHEÇA A SAPORINO" sai da abertura** e mostra a home (com header e loja), rolando para o topo. Implementação: um estado `heroGate` no roteador, **inicial `true` em todo carregamento** — cada entrada no site começa no hero; o CTA desliga o gate (`scrollTo(0,0)` + `heroGate=false`) e mostra a home. Navegar **dentro** do site de volta para `/` (logo/menu) **não** repete a abertura, porque o estado sobrevive à navegação SPA; recarregar a página mostra o hero de novo. **Sem `sessionStorage`** (decisão do dono: a entrada aparece sempre que alguém entra no site). A URL continua `/`. Por isso o hero **não** tem wordmark nem menu: o header só existe na home. (Aviso: para crawlers, `/` é a abertura — H1 do slogan + botão; a vitrine fica atrás do clique.)
