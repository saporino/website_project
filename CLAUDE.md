...
- **Multi-tenant:** `company_id` nas tabelas core, backfill p/ Saporino. **RLS LIGADA em 45/45 tabelas** (auditado 13/07/2026). O que falta NÃO é ligar RLS, e sim o **isolamento multi-tenant por `company_id`** (uma empresa não ver dados de outra) — isso é a **Camada 3** do blueprint (§15), só faz sentido quando for vender pra outras empresas.
...
**VARREDURA DE COMPLETUDE (gaps p/ "site redondo" nível 3 Corações/Melitta/Pilão — manter atualizado e ressurfar proativamente):**
- 🔴 **CRÍTICO:** (1) **E-mail transacional** — recuperação de senha ✅ FEITA (Resend+Workspace no ar); FALTA e-mail de **confirmação de compra/envio** (cliente compra e não recebe nada); (2) **RLS** ✅ LIGADA em 45/45 tabelas (13/07/2026) — falta só isolamento multi-tenant por `company_id` = Camada 3; (3) **Webhook Mercado Pago** — código pronto e deployado (`--no-verify-jwt`); FALTA confirmar no painel do MP (URL + secret) e testar 1 compra real (não há pedido B2C ainda); (4) **Performance** — code-split ✅; falta comprimir imagens de `/public` (WebP).
- 🟠 **ALTA:** (5) **busca de produtos** na loja (ausente); (6) **"Minha Conta" completa** — endereços salvos (tabela `user_addresses` existe, falta UI), **gerenciar/cancelar/pausar assinatura**, refazer pedido; (7) **cupons/código de desconto** no checkout (ausente); (8) **SEO** — sitemap.xml, robots.txt, JSON-LD Product/Organization (só há OG estático no index.html); (9) **Google Analytics/Meta Pixel** (só há `trackVisit` por IP próprio); (10) **gestão de assinantes no admin** + alerta de aniversário (brinde 6 meses) — tabela `subscriptions` já guarda commitment_months/started_at; (11) **newsletter real** (hoje `handleContactSubmit` é fake).
- 🟡 **MÉDIA:** (12) avaliações/reviews de produto; (13) modo de preparo/receitas; (14) página 404 (cai na home); (15) notificações admin de pedidos B2C + assinaturas no sininho; (16) relatórios/exportação CSV; (17) estoque→"esgotado" automático.
- 🟢 **BAIXA:** wishlist; FAQ; acessibilidade (focus trap no menu/popup, aria-live).
- **RepCo:** sólido/operacional. Ressalvas menores: Performance fica vazia até 1º pedido `status='completed'`; notificações Telegram/e-mail ausentes (push de proximidade OK).
- **PWA (quando reativar):** `start_url` está `/repco` — mudar p/ `/` e renomear manifest p/ a loja.
