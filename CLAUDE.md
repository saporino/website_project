...
- 🔴 **CRÍTICO:** (1) **E-mail transacional** — cliente compra e não recebe nada (confirmação/envio/recuperação de senha) — BLOQUEADO esperando Resend/Google Workspace; (2) **RLS** desligada — ligar antes do go-live; (3) **Webhook Mercado Pago** — confirmar ativo (pagamento aprovado precisa atualizar o pedido sozinho); (4) **Performance** — imagens de ~3MB em `/public` + bundle ~1.8MB; otimizar (WebP/compressão, lazy-load, code-split por rota).
