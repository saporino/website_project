...
import { UserCheck, ChevronDown, ChevronUp, CheckCircle, XCircle, Store, Route as RouteIcon, ArrowUp, ArrowDown, Lock } from 'lucide-react';
...
export default function PromotersAdmin() {
  ...
  async function load() {
    ...
    const { data: ps } = await supabase.from('promoters')
      .select('id,full_name,cpf,phone,email,status,company_id,created_at')
    ...
    <div className="bg-white border border-gray-200 rounded-xl">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-gray-500">Aprovar, bloquear e vincular as lojas (com gôndola) que cada promotor atende. Convite na seção acima.</p>
          </div>
        </div>
      ...
      {open && (
        <div className="border-t border-gray-100 p-4 space-y-4">
          {/* Convite de promotor agora é gerado na seção unificada acima (Tarefa 1). */}
          <div className="bg-[#f8f7f5] border border-[#ddd0cc] rounded-xl p-3 text-xs text-gray-500">
            Para convidar um promotor, use a seção <strong>“Convites (representante e promotor)”</strong> acima e escolha a função <strong>Promotor</strong>.
          </div>

          {/* Lista de promotores */}
          {promoters.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">Nenhum promotor cadastrado ainda. Gere um convite na seção “Convites” acima.</p>
          ) : (
