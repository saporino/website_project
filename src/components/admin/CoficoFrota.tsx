...
// VehicleList é reaproveitada: ownerDriverId=null => nossa frota; ownerDriverId=<id> => veículos do parceiro.
// Documentos privados em representative-docs (bucket privado com policy), sob cofico/vehicles/<id>/. Admin-only.
...
export function VehicleList({ ownerDriverId = null }: { ownerDriverId?: string | null }) {
  ...
  async function load() {
    ...
    q = ownerDriverId ? q.eq('owner_driver_id', ownerDriverId) : q.is('owner_driver_id', null);
  ...
  async function add() {
    ...
    const { error } = await supabase.from('fleet_vehicles').insert({
      ...
      owner_driver_id: ownerDriverId ?? null,
    ...
    <div className="space-y-3">
      {!showForm ? (
        <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 h-[36px] px-4 rounded-lg bg-[#8B2214] text-white text-sm font-semibold hover:bg-[#6d1a10]">
          <Plus className="w-4 h-4" /> {ownerDriverId ? 'Adicionar veículo do parceiro' : 'Adicionar veículo'}
        </button>
      ) : (
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl mb-5">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-4 py-3">
        <span className="flex items-center gap-2 font-semibold text-gray-900"><Truck className="w-5 h-5 text-[#8B2214]" /> Frota — nossos veículos</span>
        {open ? <ChevronUp className="w-5 h-5 text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}
      </button>
      {open && <div className="px-4 pb-4 border-t border-gray-100 pt-4"><VehicleList ownerDriverId={null} /></div>}
    </div>
  );
}
